...
bool ESG::BeamData::Load(	std::shared_ptr<flecs::world> _game,
							...
							GW::AUDIO::GAudio _audioEngine)
{
	...
	// other attributes
	float speed = (*readCfg).at("Beams").at("speed").as<float>();
	float xscale = (*readCfg).at("Beams").at("xscale").as<float>();
	float yscale = (*readCfg).at("Beams").at("yscale").as<float>();
	float zscale = (*readCfg).at("Beams").at("zscale").as<float>();
	int dmg = (*readCfg).at("Beams").at("damage").as<int>();
	float forwardDirection = (*readCfg).at("Beams").at("direction").as<int>();
	std::string fireFX = (*readCfg).at("Beams").at("fireFX").as<std::string>();
	...
	RegisterPrefab("Laser Beam", beamPrefab);
	
	return true;
}

bool ESG::BeamData::Unload(std::shared_ptr<flecs::world> _game)
{
