...
bool ESG::LaserData::Load(	std::shared_ptr<flecs::world> _game,
							...
							GW::AUDIO::GAudio _audioEngine)
{
	...
	float forwardDirection = (*readCfg).at("Lazers").at("direction").as<int>();
	...
	//e.add<ESG::InstanceTag>();
		...
		modelBoundary->obb.extent.x = modelBoundary->obb.extent.x / 2;
		modelBoundary->obb.extent.y = modelBoundary->obb.extent.y / 3;
		modelBoundary->obb.extent.z = modelBoundary->obb.extent.z / 2;
		modelBoundary->obb.extent.w = modelBoundary->obb.extent.w / 2;
...
bool ESG::LaserData::Unload(std::shared_ptr<flecs::world> _game)
{
