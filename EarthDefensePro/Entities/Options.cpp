...
bool ESG::OptionsData::Load(std::shared_ptr<flecs::world> _game,
	...
	GW::AUDIO::GAudio _audioEngine)
{
	...
	// other attributes
	float speed = (*readCfg).at("Options").at("speed").as<float>();
	float xscale = (*readCfg).at("Options").at("xscale").as<float>();
	float yscale = (*readCfg).at("Options").at("yscale").as<float>();
	float zscale = (*readCfg).at("Options").at("zscale").as<float>();
	//int dmg = (*readCfg).at("Options").at("damage").as<int>();
	float forwardDirection = (*readCfg).at("Options").at("direction").as<int>();
	//std::string fireFX = (*readCfg).at("Options").at("fireFX").as<std::string>();
	...
	auto optionsPrefab = _game->prefab()
...
// register this prefab by name so other systems can use it
	RegisterPrefab("Options", optionsPrefab);
	e.destruct();
	return true;
}

bool ESG::OptionsData::Unload(std::shared_ptr<flecs::world> _game)
{
