							...
							std::weak_ptr<const GameConfig> _gameConfig)
{
	...
	game->system<Bullet, Damage, Projectile>("Bullet System")
		.each([](flecs::entity e, Bullet, Damage &d, Projectile& p) {
		// damage anything we come into contact with
		e.each<Collidable>([&e, d, &p](flecs::entity hit) {
			if (hit.has<Health>() && hit != e && ((hit.has<Player>() && !p.friendly) || hit.has<Enemy>() && p.friendly)) {
